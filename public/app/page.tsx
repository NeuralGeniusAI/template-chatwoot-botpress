...
import { TITLE_PAGE, WEBHOOK_URL, AGENT_NAME } from "@/lib/constants";
...
export default function ChatInterface() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Limpiar el input y establecer estado de carga
    setInput("");
    setAttachments([]);
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      // Agregar el mensaje y el historial de conversación
      ...
      // Agregar el ID de conversación (null en la primera solicitud)
      ...
      if (userMessage.attachments && userMessage.attachments.length > 0) {
        userMessage.attachments.forEach((attachment, index) => {
          if (attachment.blob) {
            formData.append(`file_${index}`, attachment.blob, attachment.name);
          }
        });
      }

      // Enviar solicitud al webhook
      const response = await fetch(WEBHOOK_URL ? WEBHOOK_URL : "", {
        method: "POST",
        body: formData,
      });
      ...
      const assistantContent =
        responseData.output || "Lo siento, no pude procesar tu solicitud.";

      // Actualizar el ID de conversación si viene en la respuesta
      if (responseData.conversation_id) {
        setConversationId(responseData.conversation_id);
        console.log(
          "ID de conversación actualizado:",
      ...
      console.error("Error al comunicarse con el webhook:", err);
      setError(
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1">
                        <span className="text-sm font-semibold">
                          {message.role === "user" ? "Tú" : "NeuralGenius AI"}
                        </span>
